...
export function PageShell({
  ...
  return (
    <section className={className}>
      <div
        className={`mx-auto w-full max-w-7xl px-6 sm:px-10 lg:px-16 ${innerClassName}`}
      >
        {children}
      </div>
    </section>
...
export function DesignButton({
  ...
  const base =
    "inline-flex min-h-12 items-center justify-center gap-2 px-7 text-center text-[11px] font-bold uppercase tracking-[0.16em] transition duration-300 hover:-translate-y-0.5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 active:translate-y-0";
  const styles = {
    solid:
      tone === "dark"
        ? "bg-white text-black hover:bg-secondary hover:shadow-xl"
        : "bg-black text-white hover:bg-tertiary hover:shadow-xl",
    outline:
      tone === "dark"
        ? "border border-white/65 text-white hover:border-white hover:bg-white hover:text-black hover:shadow-xl"
        : "border border-black text-black hover:bg-black hover:text-white hover:shadow-xl",
    link:
      tone === "dark"
        ? "min-h-0 border-b border-white/35 px-0 py-1 text-white hover:translate-y-0 hover:border-white hover:text-secondary"
        : "min-h-0 border-b border-black/40 px-0 py-1 text-black hover:translate-y-0 hover:border-black hover:text-tertiary",
  };
    ...
    return (
      <article className="grid overflow-hidden border border-black/10 bg-neutral md:grid-cols-[1fr_0.85fr]">
        ...
        <div className="flex min-h-80 flex-col justify-center p-8 sm:p-10">
          ...
          <h3 className="font-primary text-3xl font-semibold italic leading-9 text-primary sm:text-4xl">
            {title}
          </h3>
          {author && <p className="mt-8 text-sm font-bold">{author}</p>}
          {meta && (
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary/55">
              {meta}
            </p>
          )}
          <a
            className="mt-7 inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.12em]"
            href="#"
          >
    ...
    return (
      <article className="grid overflow-hidden border border-black/10 bg-white md:grid-cols-[1.2fr_1fr]">
        <div className="flex flex-col justify-center p-8 sm:p-12">
          ...
          {description && (
            <p className="mt-6 max-w-sm leading-7 text-primary/65">
              {description}
            </p>
          )}
          <div className="mt-8">
  ...
  return (
    <article className="border border-black/10 bg-neutral">
      ...
      <div className="p-8 sm:p-9">
        <Eyebrow>{eyebrow}</Eyebrow>
        <h3 className="mt-7 font-primary text-3xl font-semibold leading-tight text-primary">
          {title}
        </h3>
        {description && (
          <p className="mt-5 leading-7 text-primary/65">{description}</p>
        )}
        {author && <p className="mt-7 text-sm font-bold">{author}</p>}
        <a
          className="mt-4 inline-flex border-b border-black text-[11px] font-bold uppercase tracking-[0.12em]"
          href="#"
        >
          Explore Story
        </a>
    ...
    <article className="group">
      ...
      <div className="mt-6 grid grid-cols-[48px_1fr] gap-5">
        <div className="font-primary text-3xl font-bold leading-7 text-white/25">
          {event.date}
          <span className="block font-secondary text-[11px] uppercase tracking-[0.16em]">
            {event.month}
          </span>
        </div>
        <div>
          <h3 className="font-primary text-2xl font-semibold text-white/75">
            {event.title}
          </h3>
          <p className="mt-3 line-clamp-2 leading-7 text-white/35">
            {event.text}
          </p>
...
export function Field({
  ...
  return (
    <label
      className={`block border-b border-black/25 ${large ? "md:col-span-2" : ""}`}
    >
      <span className="block text-[11px] font-bold uppercase tracking-[0.2em] text-primary/70">
        {label}
      </span>
      <textarea
