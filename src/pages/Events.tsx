...
import { DesignButton, EventPreviewCard, Eyebrow, PageShell } from "../components/lisa-page-sections";
    ...
    <main className="bg-black text-white">
      <PageShell className="py-24 sm:py-32">
        <div className="grid gap-10 lg:grid-cols-[1fr_0.55fr]">
          <div>
            ...
            <h1 className="mt-8 font-primary text-5xl font-semibold leading-[1.05] text-white/10 sm:text-7xl">
              What's happening <em className="font-normal">at LISA?</em>
            </h1>
          </div>
          <p className="max-w-sm self-center text-lg leading-8 text-white/40">
            A curated collection of gatherings designed to foster community, growth, and collective worship.
          </p>
        </div>

        <article className="mt-20 grid bg-neutral text-primary lg:grid-cols-[1.25fr_0.95fr]">
          <img className="h-full min-h-96 w-full object-cover" src="/images/home-bg.png" alt="" />
          <div className="p-10 sm:p-16">
            <span className="bg-[#ffe0d7] px-3 py-1 text-[11px] font-bold text-tertiary">Featured Event</span>
            <h2 className="mt-8 font-primary text-4xl leading-tight sm:text-5xl">
              <span className="mr-3 font-secondary text-3xl font-semibold">24</span>
              <em>October</em>
              <span className="mt-8 block not-italic">The Autumn Resonance: A Night of Collective Prayer</span>
            </h2>
            <p className="mt-8 max-w-md leading-8 text-primary/65">
              Join us for an immersive evening of acoustic worship and guided reflection. Experience a space designed
              for deep spiritual connection through music and light.
            </p>
            <dl className="mt-10 grid max-w-md grid-cols-2 border-y border-black/10 py-6 text-sm">
              ...
              <div>
                <dt className="text-[11px] font-bold text-primary/45">Location</dt>
        ...
        <div className="mt-28 flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex gap-10 text-[11px] font-bold text-white/35">
            <button>Worship</button>
            <button>Community</button>
            <button>Outreach</button>
          </div>
          <label className="flex items-center gap-3 border-b border-white/35 pb-3 text-white/45">
            <input className="w-56 bg-transparent text-sm outline-none placeholder:text-white/45" placeholder="Search events..." />
            <Search size={18} />
          </label>
        </div>
      ...
      <PageShell className="bg-neutral py-24 text-primary">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="font-primary text-4xl font-semibold italic sm:text-5xl">Stay in the loop</h2>
          <p className="mt-8 leading-8 text-primary/65">
            Receive a curated monthly digest of LISA events, stories, and reflections directly in your inbox.
          </p>
          <form className="mx-auto mt-12 flex max-w-md flex-col gap-5 sm:flex-row">
            <input className="min-h-12 flex-1 border-b border-black/20 bg-transparent outline-none" placeholder="Email address" />
